...
function ColorCard(props) {
  ...
  const hclC = hcl(color);
  const rgbC = rgb(color);

  const colorName = ClosestColor(hclC.toString()).name;
  ...
  const textClasses = classNames({
    ...
    'color-card__text_bright': hclC.l > 50,
    'color-card__text_dark': hclC.l <= 50
  });
